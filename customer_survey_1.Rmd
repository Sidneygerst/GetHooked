...
age_pie_df <- demo_df %>% 
  ...
  arrange(desc(age)) %>% # arragne table by descending ages so that labels are put in the right area
  mutate(proportion = total/324) %>% # calculate proportions
  mutate(prop = round(proportion*100, 2)) %>% # turn into percentage and round to 2 decimal points
  mutate(ypos = cumsum(total) - 0.5*total) # position labels across y-axis
